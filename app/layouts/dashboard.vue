...
const navItems = computed(() => [
  ...
  { icon: 'plus', label: 'Créer', path: '/dashboard/mon-club/events/new' },
  { icon: 'calendar', label: 'Événements', path: '/dashboard/events' },
  { icon: 'user', label: 'Profil', path: isClub.value ? '/dashboard/mon-club' : '/dashboard/profil' }
])
...
function isActive(path: string) {
  ...
  if (path === '/dashboard/mon-club') {
    return route.path === '/dashboard/mon-club'
  }
    ...
    <nav class="fixed bottom-0 w-full max-w-full bg-white border-t border-[#d6d6d6] py-2 z-50">
      <div class="flex items-center justify-around">
        ...
        >
          ...
          <span class="text-xs font-roboto">{{ item.label }}</span>
        </NuxtLink>
      </div>
    </nav>
  </div>
